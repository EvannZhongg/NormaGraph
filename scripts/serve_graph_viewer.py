from __future__ import annotations

import argparse
from functools import partial
import http.server
from pathlib import Path
from urllib.parse import urlencode
import webbrowser

PROJECT_ROOT = Path(__file__).resolve().parents[1]


def _resolve_derived_dir(path: Path) -> Path:
    resolved = path if path.is_absolute() else (PROJECT_ROOT / path).resolve()
    if resolved.name == 'derived':
        return resolved
    candidate = resolved / 'derived'
    if candidate.exists():
        return candidate
    return resolved


def _relative_url(path: Path) -> str:
    return path.relative_to(PROJECT_ROOT).as_posix()


def _root_relative_url(path: Path) -> str:
    return '/' + _relative_url(path).lstrip('/')


def build_viewer_url(host: str, port: int, derived_dir: Path | None, title: str | None) -> str:
    base = f'http://{host}:{port}/viewer/index.html'
    if derived_dir is None:
        return base
    nodes = derived_dir / 'graph_nodes.json'
    edges = derived_dir / 'graph_edges.json'
    requirements = derived_dir / 'requirements.json'
    params = {
        'nodes': _root_relative_url(nodes),
        'edges': _root_relative_url(edges),
    }
    if requirements.exists():
        params['requirements'] = _root_relative_url(requirements)
    if title:
        params['title'] = title
    return base + '?' + urlencode(params)


def main() -> None:
    parser = argparse.ArgumentParser(description='Serve the standalone graph viewer without using the FastAPI backend.')
    parser.add_argument('--artifact-dir', help='Artifact directory or derived directory to preload in the viewer.')
    parser.add_argument('--host', default='127.0.0.1')
    parser.add_argument('--port', type=int, default=8030)
    parser.add_argument('--title', help='Optional viewer title shown as the source label.')
    parser.add_argument('--no-open', action='store_true', help='Do not automatically open the browser.')
    args = parser.parse_args()

    derived_dir = _resolve_derived_dir(Path(args.artifact_dir)) if args.artifact_dir else None
    if derived_dir and not derived_dir.exists():
        raise SystemExit(f'derived directory was not found: {derived_dir}')

    handler = partial(http.server.SimpleHTTPRequestHandler, directory=str(PROJECT_ROOT))
    server = http.server.ThreadingHTTPServer((args.host, args.port), handler)
    url = build_viewer_url(args.host, args.port, derived_dir, args.title)
    print(f'Graph viewer is available at: {url}')
    if not args.no_open:
        webbrowser.open(url)
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print('\nShutting down graph viewer server...')
    finally:
        server.server_close()


if __name__ == '__main__':
    main()
